...
class TestInsertCryptoData(unittest.TestCase):
    ...
    @patch('etl.get_crypto_symbols', return_value=['BTC', 'ETH'])  # Mock API response for symbols
    @patch('etl.get_crypto_price', side_effect=[('BTC', 50000.0), ('ETH', 4000.0)])  # Mock API price data
